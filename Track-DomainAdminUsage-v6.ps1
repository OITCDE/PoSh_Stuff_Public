...
"@
    ...
    foreach ($priv in ($PrivilegeFrequency | Select-Object -ExpandProperty Name | Sort-Object)) {
...
"@
    ...
    foreach ($event in $PrivilegedEventsForTable) {
        ...
        $privilegesForFilter = '|' + (($event.PrivilegeList | ForEach-Object { $_.ToLower() }) -join '|') + '|'
...
$HtmlReport += @"
    ...
    <script>
        ...
        function filterPrivilegeTable() {
            ...
            const selectedDC = privDCFilter ? privDCFilter.value.toLowerCase() : '';
            const selectedPrivilege = privPrivilegeFilter ? privPrivilegeFilter.value.toLowerCase() : '';
            ...
            for (let row of privRows) {
                ...
                const rowText = row.textContent.toLowerCase();
                const rowDC = row.getAttribute('data-dc') ? row.getAttribute('data-dc').toLowerCase() : '';
                const rowPrivileges = row.getAttribute('data-privileges') ? row.getAttribute('data-privileges').toLowerCase() : '';
                ...
                if (selectedPrivilege !== '') {
                    matchesPrivilege = rowPrivileges.includes('|' + selectedPrivilege + '|');
                }
